...
# TAG_FORGEHOOK
TAG_FORGEHOOK="forgehook-repeat"
...
if [[ "$PHASE" == "pre-start" ]]; then
    ...
    # Add Tags FORGEHOOK 
    if ! grep -qE "^tags:[[:space:]]*([^#]*[;,[:space:]])?${TAG_FORGEHOOK}([;,[:space:]]|$)" "$CONF"; then
        if grep -qE '^[[:space:]]*tags:' "$CONF"; then
            sed -i "s/^[[:space:]]*tags:[[:space:]]*\(.*\)$/tags: \1;${TAG_FORGEHOOK}/" "$CONF"
        else
            echo "tags: ${TAG_FORGEHOOK}" >> "$CONF"
        fi
